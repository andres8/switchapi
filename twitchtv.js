$(document).ready(function(){
	...
	for (let i = 0; i<channels.length; i++){
		...
		$.getJSON(d[i],function(donnes){
			if(donnes.stream){
				html += "<div class='row mt-2'><div class='col-sm-4 align-self-center'><img class='imagen' src='"+donnes.stream.channel.logo+
				...
				"</a><p class='status'>"+donnes.stream.channel.status+"</p></div><div class='col-sm-2 align-self-center'><i class='fa fa-check check' style='font-size:24px;color:green'></i></div></div>"
					}
			else{
				$.getJSON("https://wind-bow.glitch.me/twitch-api/channels/"+channels[i],function(data){
					html += "<div class='row mt-2'><div class='col-sm-4 align-self-center'><img class='imagen' src='"+data.logo+
					...
					"</a><p class='status'>This channel is currently offline</p></div><div class='col-sm-2 align-self-center'><i class='fa fa-exclamation-triangle' style='font-size:24px;color:red'></i></div></div>"
					$("#sortie").html(html);
					});
				}
			});
